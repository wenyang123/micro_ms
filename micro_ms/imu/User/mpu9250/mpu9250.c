...
void MPU9250_Mode_Init(void)
{
	...
	_MPU9250.x_accel=65536.0f/16.0f;
	_MPU9250.x_gyro=65536.0f/4000.0f;
	_MPU9250.imu_PIc=180.f/PI;
}
...
void MPU9250_Mode_Read_Gyro(int16_t *gyrodata)
{
    ...
    gyrodata[0] = (int16_t)(((buf[0] << 8) | buf[1])-_MPU9250.gyro_Check[0]);   
    gyrodata[1] = (int16_t)(((buf[2] << 8) | buf[3])-_MPU9250.gyro_Check[1]);   
    gyrodata[2] = (int16_t)(((buf[4] << 8) | buf[5])-_MPU9250.gyro_Check[2]);
}
...
void MPU9250_Mode_Read_Acc(int16_t *accdata)
{
    ...
    accdata[0] = (int16_t)(((buf[0] << 8) | buf[1])-_MPU9250.acc_Check[0]);   
    accdata[1] = (int16_t)(((buf[2] << 8) | buf[3])-_MPU9250.acc_Check[1]);   
    accdata[2] = (int16_t)(((buf[4] << 8) | buf[5])-_MPU9250.acc_Check[2]);
}
...
{ 
	uint8_t i;
	MPU9250_Mode_Read_Acc(_MPU9250.accADC);
	MPU9250_Mode_Read_Gyro(_MPU9250.gyroADC);
	...
	for(i=0;i<3;i++)
		{
			_MPU9250.accGet[i]= (float)_MPU9250.accADC[i]  /_MPU9250.x_accel;
			_MPU9250.gyroGet[i]=(float)_MPU9250.gyroADC[i] /_MPU9250.x_gyro;	
		}

	_MPU9250.accData[0]=LPF2p_Apply(&LPF_Acc_X,_MPU9250.accGet[0]);
	_MPU9250.accData[1]=LPF2p_Apply(&LPF_Acc_Y,_MPU9250.accGet[1]);
	_MPU9250.accData[2]=LPF2p_Apply(&LPF_Acc_Z,_MPU9250.accGet[2]);
#if 1
	_MPU9250.gyroData[0]=LPF2p_Apply(&LPF_Gyro_X,_MPU9250.gyroGet[0]);
	_MPU9250.gyroData[1]=LPF2p_Apply(&LPF_Gyro_Y,_MPU9250.gyroGet[1]);
	_MPU9250.gyroData[2]=LPF2p_Apply(&LPF_Gyro_Z,_MPU9250.gyroGet[2]);
#else
	_MPU9250.gyroData[0]=_MPU9250.gyroGet[0];
	_MPU9250.gyroData[1]=_MPU9250.gyroGet[1];
	_MPU9250.gyroData[2]=_MPU9250.gyroGet[2];
#endif
}
...
void MPU9250_Mode_MeCheck(void)
{
	...
	for(g=0;g<100;g++)
	{
			MPU9250_Mode_Read_Acc(_MPU9250.accADC);MPU9250_Mode_Read_Gyro(_MPU9250.gyroADC);
			Acc_check[0]  +=_MPU9250.accADC[0];
			Acc_check[1]  +=_MPU9250.accADC[1];
			Acc_check[2]  +=_MPU9250.accADC[2]-4096;
			Gyro_check[0] +=_MPU9250.gyroADC[0];
			Gyro_check[1] +=_MPU9250.gyroADC[1];
			Gyro_check[2] +=_MPU9250.gyroADC[2];
		  delay_ms(1);

	}
		  _MPU9250.acc_Check[0]  =(int32_t)Acc_check[0]/100;
			_MPU9250.acc_Check[1]  =(int32_t)Acc_check[1]/100;
			_MPU9250.acc_Check[2]  =(int32_t)Acc_check[2]/100;
	
#if 1
		  _MPU9250.gyro_Check[0]  =(int32_t)Gyro_check[0]/100;
			_MPU9250.gyro_Check[1]  =(int32_t)Gyro_check[1]/100;
			_MPU9250.gyro_Check[2]  =(int32_t)Gyro_check[2]/100;
#endif
